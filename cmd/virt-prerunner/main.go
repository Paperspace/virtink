...
func setupBridgeNetwork(linkName string, ipv4CIDR string, netConfig *cloudhypervisor.NetConfig) error {
	...
	if linkIPv6Addr != nil {
		clientLLA := ipv6util.GenerateEUI64Address(net.ParseIP("fe80::0"), linkMAC)
		if _, err := executeCommand("ip6tables", "-A", "INPUT", "-i", bridgeName, "!", "-s", clientLLA.String(), "-p", "udp", "-m", "multiport", "--sports", "546", "-j", "DROP"); err != nil {
...
func setupMasqueradeNetwork(linkName string, ipv4CIDR string, ipv6CIDR string, netConfig *cloudhypervisor.NetConfig) error {
	...
	if linkIPv6Addr != nil {
		clientLLA := ipv6util.GenerateEUI64Address(net.ParseIP("fe80::0"), vmMAC)
		if _, err := executeCommand("ip6tables", "-A", "INPUT", "-i", bridgeName, "!", "-s", clientLLA.String(), "-p", "udp", "-m", "multiport", "--sports", "546", "-j", "DROP"); err != nil {
			return fmt.Errorf("allow DHCPv6 request only from client: %s", err)
		}
		...
		command := exec.Command("rad", "-interface", bridgeName, "-router", bridgeIPv6LLA.IP.String(), "-client", clientLLA.String(), "-prefix", vmIPv6Net.String())
		command.Stdout = os.Stdout
		command.Stderr = os.Stderr
		if err := command.Start(); err != nil {
			return fmt.Errorf("start RAD: %s", err)
